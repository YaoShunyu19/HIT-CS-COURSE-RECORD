...
parser.add_argument('--anchor', type=str, default='gan', help='model name')
...
parser.add_argument('--g_step_num', type=int, default=8, help='g_step_num')
...
def main():
    ...
    for iteration in range(opt.iteration_num):
        ...
        for g_step in range(opt.g_step_num):
            ...
            # g_loss = -torch.mean(torch.log(1. - output))
            g_loss.backward()
        ...
        for d_step in range(opt.d_step_num):
            ...
            for i, data in enumerate(train_ld):
                ...
                # loss_real = -torch.mean(torch.log(1. - output))
                loss_real.backward()
                ...
                # loss_fake = -torch.mean(torch.log(output))
                loss_fake.backward()
