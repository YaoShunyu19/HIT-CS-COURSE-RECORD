...
class Detect(object):
    ...
    def cc_fast_nms(self, boxes, masks, scores, iou_threshold: float = 0.5, top_k: int = 200):
        ...
        scores, classes = scores.max(dim=0)  # 计算每列最大的分
        _, idx = scores.sort(0, descending=True)  # 将得分按照降序排列
        idx = idx[:top_k]  # 取得分最大的K个
        boxes_idx = boxes[idx]
        iou = jaccard(boxes_idx, boxes_idx).triu_(diagonal=1)  # 计算每个box的交集大小
        maxA, _ = torch.max(iou, dim=0)  # 取出iou中每列的最大值
        idx_out = idx[maxA <= iou_threshold]  # 取小于阈值的框为输出结果
    ...
    def cc_cluster_SPM_nms(self, boxes, masks, scores, iou_threshold: float = 0.5, top_k: int = 200):
        ...
        for i in range(200):
            ...
            if A.equal(B) == True:
                break
        scores = torch.prod(torch.exp(-B ** 2 / 0.2), 0) * scores  # 将得分进行加权
        idx_out = scores > 0.01
        return boxes[idx_out], masks[idx_out], classes[idx_out], scores[idx_out]
    ...
    def cc_cluster_diounms(self, boxes, masks, scores, iou_threshold: float = 0.5, top_k: int = 200):
        ...
        idx = idx[:top_k]
        boxes_idx = boxes[idx]
        iou = diou(boxes_idx, boxes_idx).triu_(diagonal=1)  # 计算iou时考虑中心之间的距离
        B = iou
        for i in range(200):
            A = B
            maxA, _ = torch.max(A, dim=0)
    ...
    def cc_cluster_nms(self, boxes, masks, scores, iou_threshold: float = 0.5, top_k: int = 200):
        ...
        iou = jaccard(boxes_idx, boxes_idx).triu_(diagonal=1)
        B = iou
        # 该循环改变了fast-nms中简单的取maxA的方式，可以防止fast-nms中多删除box
        for i in range(200):
            A = B
            maxA, _ = torch.max(A, dim=0)
            E = (maxA <= iou_threshold).float().unsqueeze(1).expand_as(A)
