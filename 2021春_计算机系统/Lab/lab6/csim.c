...
cache_t cache;
mem_addr_t set_index_mask;//鐢ㄤ簬浠庡唴瀛樺湴鍧€寰楀埌缂撳瓨缁勭殑绱㈠紩鍊?
...
void accessData(mem_addr_t addr)
{
    ...
    for (j=0;j<E;j++) {
        if (cache[set_index][j].valid=='1' && cache[set_index][j].tag==tag) {
            hit_count++;
            lru_counter++;
            cache[set_index][j].lru=lru_counter;
            return;
        }
    }
    miss_count++;
    ...
    if (whether_full==1) {
        mem_addr_t minlru=((unsigned long long int)-1)>>1;
            for (j=0;j<E;j++) {
