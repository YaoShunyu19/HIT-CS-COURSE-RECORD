...
{	
	sigset_t mask;
	if (sigemptyset(&mask)<0) {    /* 清空block */
		unix_error("sigemptyset error");
	}
	while(pid == fgpid(jobs)) {
		sigsuspend(&mask);  /* 用mask替换当前的阻塞集合，然后挂起该进程 */ 
	}
    return;
}
...
void sigint_handler(int sig) 
{
	...
	if (sigaddset(&mask,sig)<0) {  /* 将信号SIGTSTP加入集合 */ 
		unix_error("sigaddset error");
	}
	if (sigprocmask(SIG_BLOCK,&mask,&prev_mask)<0) {  /* 临时阻塞接受信号 */
		unix_error("sigprocmask error");
	}
	pid_t fg_pid=fgpid(jobs);  /* 获取前台作业pid */
	if (sigprocmask(SIG_SETMASK,&prev_mask,NULL)<0) {  /* 解除阻塞接受信号 */
		unix_error("sigprocmask error");
	}
	if (fg_pid!=0) {
		kill(-fg_pid,SIGINT);  /* 将信号SIGINT传递给子进程 */ 
	}
	errno=olderrno;  /* 恢复错误状态 */
    return;
}
...
void sigtstp_handler(int sig) 
{
	...
	if (fg_pid!=0) {
		kill(-fg_pid,SIGTSTP);  /* 将信号SIGTSTP传递给子进程 */ 
	}
	errno=olderrno;  /* 恢复错误状态 */
    return;
}
